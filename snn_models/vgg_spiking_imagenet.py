...
class VGG_SNN_STDB_IMAGENET(nn.Module):
	...
	def __init__(self, vgg_name, activation='Linear', labels=10, timesteps=100, leak=1.0, default_threshold = 1.0,
				 alpha=0.3, beta=0.01, dropout=0.2, kernel_size=3, dataset='CIFAR10', device='cuda',
		...
		# self.matrixMean = tmpmean.expand(3, imgSize, imgSize)
		# tmpstd = torch.Tensor(np.array(std)[:, np.newaxis, np.newaxis])
		# self.matrixStd = tmpstd.expand(3, imgSize, imgSize)
		# # self.matrixMean = self.matrixMean.to(self.device)
		# self.matrixMean = self.matrixMean.cuda()
		# # self.matrixStd = self.matrixStd.to(self.device)
	...
	def neuron_init(self, x):
		...
		for l in range(len(self.features)):
			...
			if isinstance(self.features[l], nn.Conv2d):
				self.mem[l] 		= torch.zeros(self.batch_size, self.features[l].out_channels, self.width, self.height).to(self.device)
			
			elif isinstance(self.features[l], nn.ReLU):
				if isinstance(self.features[l-1], nn.Conv2d):
					self.spike[l] 	= torch.ones(self.mem[l-1].shape).to(self.device)*(-1000)
				elif isinstance(self.features[l-1], nn.AvgPool2d):
					self.spike[l] 	= torch.ones(self.batch_size, self.features[l-2].out_channels, self.width, self.height).to(self.device)*(-1000)
			
			elif isinstance(self.features[l], nn.Dropout):
				self.mask[l] = self.features[l](torch.ones(self.mem[l-2].shape).to(self.device))
		...
		for l in range(len(self.classifier)):
			...
			if isinstance(self.classifier[l], nn.Linear):
				self.mem[prev+l] 		= torch.zeros(self.batch_size, self.classifier[l].out_features).to(self.device)
			
			elif isinstance(self.classifier[l], nn.ReLU):
				self.spike[prev+l] 		= torch.ones(self.mem[prev+l-1].shape).to(self.device)*(-1000)

			elif isinstance(self.classifier[l], nn.Dropout):
				self.mask[prev+l] = self.classifier[l](torch.ones(self.mem[prev+l-2].shape).to(self.device))
	...
	def forward(self, x, mem=0, spike=0, mask=0, find_max_mem=False, max_mem_layer=0):
		...
		#pdb.set_trace()
		imgSize = 224
		tmpmean = torch.Tensor(np.array(self.mean)[:, np.newaxis, np.newaxis])
		self.matrixMean = tmpmean.expand(3, imgSize, imgSize)
		tmpstd = torch.Tensor(np.array(self.std)[:, np.newaxis, np.newaxis])
		self.matrixStd = tmpstd.expand(3, imgSize, imgSize)
		self.matrixMean = self.matrixMean.to(self.device)
		# self.matrixMean = self.matrixMean.cuda()
		self.matrixStd = self.matrixStd.to(self.device)
		...
		if len(self.classifier) > 1:
			...
			# return self.mem[prev], self.mem, self.spike, self.mask
