...
class RESNET_SNN_STDB(nn.Module):
    ...
    def __init__(self, resnet_name, activation='Linear', labels=10, timesteps=75, leak=1.0, default_threshold=1.0,
                 alpha=0.5, beta=0.035, dropout=0.2, dataset='CIFAR10', mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010]):
    ...
    def neuron_init(self, x):
        ...
        for l in range(len(self.pre_process)):
            ...
            if isinstance(self.pre_process[l], nn.Conv2d):
                ...
                self.spike[l] = torch.ones(self.mem[l].shape).cuda() * (-1000)
                ...
                for l in range(len(layer[index].residual)):
                    if isinstance(layer[index].residual[l], nn.Conv2d):
                        self.mem[pos] = torch.zeros(self.batch_size, layer[index].residual[l].out_channels, self.width,
                                                    self.height).cuda()
                        self.spike[pos] = torch.ones(self.mem[pos].shape).cuda() * (-1000)
        ...
        for l in range(len(self.classifier)):
            if isinstance(self.classifier[l], nn.Linear):
                self.mem[pos + l] = torch.zeros(self.batch_size, self.classifier[l].out_features).cuda()
                self.spike[pos + l] = torch.ones(self.mem[pos + l].shape).cuda() * (-1000)
            elif isinstance(self.classifier[l], nn.Dropout):
                self.mask[pos + l] = self.classifier[l](torch.ones(self.mem[pos + l - 2].shape).cuda())
