import torch
from snn_lib import *              
from snn_lib_extension import neuron_layer_bntt_v2
from snn_lib_extension import conv2d_layer_bntt_v2
...
class SNNBackprop_grad_estimation(torch.nn.Module):
    #global cfg
    ...
    def __init__(self, length, batch_size, tau_m, tau_s, train_synapse_tau, train_neuron_tau, membrane_filter, neuron_model, synapse_type, dropout):
        ...
        if self.neuron_model == 'iir':
            ...
            c,h,w = self.make_layers(archConfig['VGG16'], synapse_config)
            ...
            self.layers.append(neuron_layer_bntt_v2(c*h*w, 512, self.length, self.batch_size, 
                                                self.tau_m, membrane_filter = self.membrane_filter, 
                                                return_state=False, train_decay_v=self.train_neuron_tau, **synapse_config))

            self.layers.append(torch.nn.Dropout(p=self.dropout))

            self.layers.append(neuron_layer_bntt_v2(512, 512, self.length, self.batch_size, 
                                                self.tau_m, membrane_filter = self.membrane_filter, 
                                                return_state=False, train_decay_v=self.train_neuron_tau, **synapse_config))

            self.layers.append(torch.nn.Dropout(p=self.dropout))

            self.layers.append(neuron_layer_bntt_v2(512, 100, self.length, self.batch_size,
                                                self.tau_m, membrane_filter = self.membrane_filter, 
                                                return_state=False, train_decay_v=self.train_neuron_tau, **synapse_config))
        ...
        self.output_filter = first_order_low_pass_layer((100,), self.length, self.batch_size, self.tau_s, False)
    ...
    def make_layers(self, configs, synapse_config):
        ...
        for cfg in configs:
            if 'M' == cfg:
                ...
                self.layers.append(conv2d_layer_bntt_v2(h_input=prev_h, w_input=prev_w, in_channels=prev_c, out_channels=cfg, kernel_size=3, stride=1, padding=1, dilation=1, 
    ...
    def change_threshold_func(self, threshold_func):
        for layer in self.layers:
            if isinstance(layer, neuron_layer_bntt_v2) or isinstance(layer, conv2d_layer_bntt_v2):
                layer.change_threshold_func(threshold_func)
